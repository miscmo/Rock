...
void test_gzip() {
    ...
    std::string data = rock::random_string(102400);

    auto gzip_compress = rock::ZlibStream::CreateGzip(true, 1);
    ...
    auto gzip_uncompress = rock::ZlibStream::CreateGzip(false, 1);
...
void test_deflate() {
    ...
    auto gzip_compress = rock::ZlibStream::CreateDeflate(true, 1);
    std::cout << "compress: " << gzip_compress->write(data.c_str(), data.size())
              << " length: " << gzip_compress->getBuffers().size()
              << std::endl;
    std::cout << "flush: " << gzip_compress->flush() << std::endl;
    ...
    auto comperss_str = gzip_compress->getResult();
    auto gzip_uncompress = rock::ZlibStream::CreateDeflate(false, 1);
...
void test_zlib() {
    ...
    auto gzip_compress = rock::ZlibStream::CreateZlib(true, 1);
    std::cout << "compress: " << gzip_compress->write(data.c_str(), data.size())
              << " length: " << gzip_compress->getBuffers().size()
              << std::endl;
    std::cout << "flush: " << gzip_compress->flush() << std::endl;
    ...
    auto comperss_str = gzip_compress->getResult();
    auto gzip_uncompress = rock::ZlibStream::CreateZlib(false, 1);
